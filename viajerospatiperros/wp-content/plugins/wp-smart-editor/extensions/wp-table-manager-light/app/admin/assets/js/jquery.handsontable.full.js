  ...
  this.eventManager.addEventListener(document.documentElement, 'mousedown',function (event) {
    ...
    if (that.settings.outsideClickDeselects) {
      instance.deselectCell();
        ...
        //JoomUnited: little hack in order to work witht CKeditor modal windows
...
(function(Handsontable){
  Handsontable.UndoRedo = function (instance) {
   ...
   //JoomUnited: undo
    instance.addHook("afterChangeStyle", function (changes, origin) {        
      if(changes){         
        var action = new Handsontable.UndoRedo.ChangeStyleAction(origin);
        plugin.ignoreNewActions = false;      
        plugin.done(action);
      }      
    });
    //JoomUnited: undo for merge cells
    instance.addHook("afterMergeOrUnmerge", function (selected) {  
       
        var action = new Handsontable.UndoRedo.MergeOrUnmergeAction(selected);
        plugin.ignoreNewActions = false;      
        plugin.done(action);
  ...
  //JoomUnited: undo
  Handsontable.UndoRedo.ChangeStyleAction = function (origin) {
    this.oldStyle = origin;
  };
  Handsontable.helper.inherit(Handsontable.UndoRedo.ChangeStyleAction, Handsontable.UndoRedo.Action);
  Handsontable.UndoRedo.ChangeStyleAction.prototype.undo = function (instance, undoneCallback) {
    instance.runHooks('undoChangeStyle',  this.oldStyle );
  };
  //JoomUnited: undo  mergecell
  Handsontable.UndoRedo.MergeOrUnmergeAction = function (cellRange) {
    this.oldCellRange = cellRange;
  };
  Handsontable.helper.inherit(Handsontable.UndoRedo.MergeOrUnmergeAction, Handsontable.UndoRedo.Action);
  Handsontable.UndoRedo.MergeOrUnmergeAction.prototype.undo = function (instance, undoneCallback) {
    instance.mergeCells.mergeOrUnmergeSelection(this.oldCellRange);
    instance.render();
    instance.runHooks('afterChange', false, 'mergeCells');
  };
...
var addMergeActionsToContextMenu = function (defaultOptions) {
  ...
  defaultOptions.items.push({
    ...
    callback: function () {
      // Joomunited : for undo merge
      var sel =  this.getSelectedRange();    
      this.mergeCells.mergeOrUnmergeSelection( sel );
      this.render();                      
      this.runHooks('afterMergeOrUnmerge', sel);
      this.runHooks('afterChange', false, 'mergeCells');
    },
    disabled: function () {
      return false;
    }
